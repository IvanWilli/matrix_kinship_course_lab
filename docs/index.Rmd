...
Let's take a look at the resulting kin counts from a time-variant model for a Focal born in 1960, limiting the output to a selection of relatives (see argument `output_kin`) and a given cohort (argument `output_cohort`).
...
```{r, eval=T}
read.csv('locations.csv', sep='|', skip=1) %>% 
...
**1.1** Using the `data` object created above, for your own country, reshape the fertility and survival data in the same way in which we reshaped the fertility data above. Plot the $q_x$ values over age for all periods available in the same way we did for Sweden (use different colours for each period).
...
1. You will need a 'baseline scenario': what would mortality have looked like in 2020 and 2021 in the absence of COVID? Create a create a counterfactual 'non-covid' scenario by assuming that the mortality levels observed in 2019 (i.e. before the pandemic) were in effect during 2020-2021. (You can also create more sophisticated baselines if you want.) 
...
1. Second, run two time-variant kinship models: one 'realistic' model that uses the empirical mortality and fertility rates for the 1950:2021 period. The 'counterfactual' models is equivalent to the 'realistic' one except that it uses the 2019 mortality rates for the 2020:2021 period.
1. Estimate the difference between the number of living kin for a Focal aged 60 across the 'realistic' and 'counterfactual' scenarios for the year 2021.
