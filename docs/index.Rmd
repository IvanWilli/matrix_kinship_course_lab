...
`DemoKin` allows the computation of kin structures in a multi-state framework, classifying individuals jointly by age and some other feature (e.g., stages of a disease). For this, we need mortality and fertility data for each possible stage and probabilities of of transitions between stages by age.
...
`DemoKin` includes the data to replicate this analysis for the year 1990. Note that the function allows that user can entry lists (should be consistent in dimensions), as is shown in the paper, or in a matrix format for some arguments. The data that comes with the package is in this format:
...
With the function `kin_multi_stage` we can obtain the joint age-parity kin structure in a time-invariant framework. By now, run the model and inspect the output, we´ll get into details in next section:
...
**E1**. What is the distribution by parity of the mother when Focal is 30 yo?
