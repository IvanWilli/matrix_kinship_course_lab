...
You can use a function from package `expm`. For a random matrix called X: 
...
<!-- It has years in columns and age in rows. We thought that from an user perspective it could be more easy to organize the input rather than include $U$. Plotting $q_x$ ($´s complement) over age in a year, for example 2015, looks like this: -->
...
<!-- Plotted over time and age the same year: -->
...
<!-- Is coherent the mode of each kin distribution with Focal's age? Take a moment and do this bivariate comparison by intuition reviewing each type. -->
...
<!-- > All exercises can be completed using datasets included in `DemoKin`. Yes, last day with Sweden, we promise. -->
...
<!-- **1.3**: Plot the 'Keyfitz' diagram for that woman and compare visually with the one from previous section (which were based on rates from year 2015). -->
...
<!-- **1.4**: What is the cumulative number of offspring deaths experienced by an average woman who survives to age 65? Compare it with the 2015 scenario creating a data frame with columns: `kin`, `loss_1910`, `loss_2015`. Which of the two Focals (one experiencing the 1910 rates and the other the 2015 rates) has lost more daughters at age 65? How many more daughters has she lost? -->
...
<!-- Compute the mean and standard deviation of the age of sisters over the ages of Focal **by your own** (i.e., using the raw output in `kin_full`) and: -->
...
<!-- **2.2**: Do the same with rates from 1910 and compare. Think in a good way to visualize the comparison and try it. -->
...
<!-- For this exercise, we consider the level 'sandwich-ness' to the sum of living daughters aged $15$ or younger and mothers (actually the portion of her) older than $75$ (in the paper this age $a$ is the one that makes $e_a=5$). Use `DemoKin` to find at which age Focal is more *sandwiched* in 2015 under previous definition, assuming time-invariant rates and a female-only population. If you plot the total 'sandwich-ness' by age you get 500 points, but if also color each kin contribution you get 1,000 points. -->
...
<!-- #### 3. By hand calculation -->

<!-- **3.1**  -->

<!-- Use vectors $p$ and $f$ from the toy example in *By Hand* section, but this time with `DemoKin`. Do you have the same results? How many grandmothers have Focal during her life? -->
...
<!-- To interpret the results ...."plot apc" -->
...
<!-- These are the living kin that expects to have an average woman born in 1960, given experienced fertility, mortality and population distribution since 1900 to 2018. It is important to distinguish between time boundary restriction and cohort year: with `output_cohort = 1960` we are sub-setting a Focal cohort (a diagonal) from all possible results (101 age-classes and 119 years). Estimates stop at age 58 because of input restriction up to 2018. -->
...
<!-- But also we can compare cohorts, for example against 1910`s one. Take a time to interpret results (table and plot). **Answer**: Do you notice any unexpected trend for some kin? What may be causing it? -->
...
<!-- Maybe you are interested in take a snapshot of kin distribution in some year, for example 1960. Set argument`output_period = 1960`. **Answer**: looks similar to previous plots? What can you describe and not with this period approach, and what with the cohort approach?  -->
...
<!-- The function `kin` also includes information on the number of relatives lost by Focal during her life, stored in the column `count_cum_death`. Let's follow with the cohort comparison 1910 vs 1960: which kin types looks that have the biggest differences? See it in absolute and relative terms (*free* scale in this grid plots can be confusing some times when comparing counts with non similar magnitudes). -->
...
<!-- **1.1** Reshape data for survival in the same way and visualize it. -->
...
<!-- **1.2** Run a time-variant model for your country, selecting years 1950 and 2015 from a period perspective. Focusing in Focal age 50, compare both years in terms of total living and and mean age, and extract two or three findings. -->
...
<!-- **1.2** Run a time-variant model for your country selecting cohort 1990. If we assume that each kin lives all the interval and dies (if does) at the end of the year, this means that each lived year by each kin was a "shared" one, so you can sum kin availability and get an approximation of accumulated person-years. Estimate the total amount of sister-years shared since birth for a Focal aged 15, interpret the result and find its limitations. -->

<!-- > Take a look to the graphic from @Song2019 for shared grandchild-years from a grandparent perspective, we are in the definition number 2. -->
...
<!-- ``` -->
<!-- **E.1.4** Covid pandemic had a big impact in kin bereavement for some countries (@verdery_tracking_2020; @Snyder2022). If you make an scenario replacing 2020 and 2021 survival probabilities by the ones from 2019 (one choice of baseline, could be averaging some previous years too), you can have a sense of Covid19 impact in kin bereavement. Calculate the difference between living older kin for a female aged 60 in the two scenarios. But! We recommend to take a look on excess mortality implied in survival pattern to know in advance the magnitude of the shock you can expect. -->
...
<!-- E.3 Run the toy example with DemoKin and check if you have the same results for *mother* (the one that we coded in that section). To make it work you need first to add row names (age) and column names (years) to $Pt$ and $Ft$. Remember to set `birth_female = 1` and set also the argument `pi = Pi` to reflect better the calculations. -->
