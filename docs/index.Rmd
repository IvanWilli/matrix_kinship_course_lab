...
```
**E1.2** Compare sex ratios of grandparents, parents, daughters and siblings in a time-variant framework for the cohort 1950, at each of Focal.
...
Data for female mortality and fertility are already included in `DemoKin`. For males you can access to a file called *"swe_males.Rdata"* in the data folder, with age-specific survival probabilities and fertility rates. Where comes from this data? See:

  - Male fertility can be downloaded from [Human Fertility Collection](https://www.fertilitydata.org/Home/Index) site, directly from R with sentence `read.table("https://www.fertilitydata.org/File/GetFile/Country/males/SWE/m_SWE_ASFRstand_TOT.txt", sep = ",", header = T)`. The range of years is 1751-2020.
...
1. When arranging inputs for using in `DemoKin`, be aware that female and male data have different window range. For this exercise take the *interval* where you have data for *both* (1968-2015).

2. For the sake of simplicity assume [@kolk_swedish_2021] estimate is in 2015. So...

```{r}
kolk$cohort <- kolk$cohort-2
```

3.  A table for help in using GKP factors and matching kin names with [@kolk_swedish_2021]:
```{r}
...
We see there are some issues with data in parents and cousins, so we'll left those out of the comparison. We get an average difference of total living kin, assuming all Focal ages have the same weight (not population weighted). It looks that GKP model is closer, followed by full and then the androgynous one. 

```{r}
df_comparison %>%
  filter(!kolk_kin %in% c("cousins", "parents")) %>% 
...
<!-- `DemoKin` includes the data to replicate this analysis for the year 1990. Note that the function allows that user can entry lists (should be consistent in dimensions), or in a matrix format for inputs `f`, `D` and `H`. The data that comes with the package is in this format:  -->
...
<!-- # at age 31 -->
<!-- svk_Uxs[[30]] -->
<!-- colSums(svk_Uxs[[30]]) #column-stochastic -->
<!-- ``` -->
...
<!-- class(svk_fxs) -->
<!-- dim(svk_fxs) -->
<!-- plot(0:109, svk_fxs[,1], col=1, t="l", ylim = c(0,.15), xlim=c(0,55)) -->
<!-- for (i in 2:6) lines(svk_fxs[,i], col=i) -->
<!-- legend("topright", c("0","1","2","3","4","5+"), col=1:6, lty=1) -->
<!-- ``` -->
...
<!-- With function `kin_multi_stage` can obtain the joint age-parity kin structure in a time-invariant framework: -->
...
<!-- **Answer**: What other kind of indicators are interesting in this context? -->
...
<!-- Replicate previous results using the function `kin_multi_stage.R`, taht can be sourced from [here](https://github.com/IvanWilli/matrix_kinship_course_lab/tree/main/docs). Set as iarguments `U`, `f`, `H` and `D` the objects already created before, given that the function can handle lists and we already created those: -->
...
<!-- Also is needed to set `birth_female = 1` because fertility are already for female offspring, and the analysis is not on parity distribution, so `parity = FALSE`. -->

<!-- ```{r, include=F} -->
<!-- source("kin_multi_stage.R") -->
<!-- debugonce(kin_multi_stage) -->
<!-- U <- list(U_0, U_1, U_2, U_3, U_4) -->
<!-- H <- list(H_low, H_high) -->
<!-- D <- list(D_low, D_high) -->
<!-- F. <- lapply(1:ages, function(x){ -->
