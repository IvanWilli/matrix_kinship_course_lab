...
Let's project that population (here the *key* insight is that each kin constitutes a population) and get the living mothers at each age of Focal, preserving the age's kin too (that's why we save results in column vectors on the matrix).
...
For the case of the **daughters** of Focal, the initial count of living kin is zero (all women have zero daughters when they are born).

```{r}
A <- matrix(0, 5, 5) # eq 7 in Caswell (2019)
```

Let's project that population, adding offspring from Focal at each age. We create the diagonal matrix $E$ for this purpose:
...
Take a moment to understand how the multiplication between $F$ and $E[,x]$ works.
...
> Note: For now, we are assuming a female [matrilineal](https://en.wikipedia.org/wiki/Matrilineality) population, so Focal has only one grandmother from her mother's side.
...
```{r}
plot(age, d, t = "b", ylab = "k", ylim = c(0, 1.5))
...
It has years in columns and age in rows. We thought that from an user perspective it could be more easy to organize the input rather than include $U$. Plotting $q_x$ ($´s complement) over age for 2015 gives:
...
Compute the mean and standard deviation of the age of sisters over the ages of Focal **by hand** (i.e., using only the raw output in `kin_full`) and:
...
**2.2**: Do the same with rates from 1910 and compare. Compare both sets of results visually.
...
**Answer:** At which age is Focal more *sandwiched* (i.e. at which age does she simultaneously have the highest number of dependent daughters and mothers)?

If you plot the total 'sandwich-ness' by age you get 500 points, but if also color each kin contribution you get 1,000 points.
...
Replicate the results for the expected number of grandmothers using `DemoKin`. **Answer**: Do you get the expected number of grandmothers for Focal using DemoKin as you did in the [By hand](#by_hand_mon8) section?
...
In this case we have access to a vector of female population `pop` at the beginning of the period, so we can project this and calculate empirical mother's distribution by age for that year and next the ones:
...
<!-- DAG: To interpret the results .... add "plot apc" -->
...
**1.1** Using the `data` object created above, reshape the survival data in the same way in which we reshpared the fertility data above. The qx values over age for all periods available in the same plot (use different colours for each period).
...
1. Using these two models, estimate difference between living older kin for a female aged 60 in the 'realistic' and 'counterfactual' scenarios. 
...
#### 4. Compare DemoKin to calculations 'by hand'

Replicate the toy example presented in the *By Hand* section (at the start of the tutorial for today) using `DemoKin`. Check whether you can get the same results for *mothers*. Note that, to make this work, you will need to first add row names (age) and column names (years) to $Pt$ and $Ft$. In order to make the comparisson more realistic, remember to set `birth_female = 1` and set also the argument `pi = Pi` in the `DemoKin::kin` function.
...
<!-- **E1** You will use data on kinship structures [@kolk_swedish_2021] to benchmark formal models of kinship. -->
...
<!-- **E1.1** How well do your models approximate the kin counts reported by Kolk et al. (2021)? Justify your answer using quantitative measures (max 250 words and 4 figures) -->

<!-- ```{r} -->
<!-- # Write your code here -->
<!-- ``` -->

<!-- **E1.1** Based on this exercise, discuss the advantages and drawbacks of using formal models to quantify kinship (max 250 words) -->

<!-- ```{r} -->
<!-- # Write your code here -->
<!-- ``` -->

<!-- **E2** Run the toy example but with `DemoKin` and check if you have same results for mother and daughters. -->
...
<!-- mean_ages_2015 <- swe_2015$kin_full %>% -->
<!--   filter(age_focal == 35) %>% -->
<!--   summarise(mean_age = sum(age_kin*living)/sum(living), .by = c(kin)) -->
<!-- mean_ages_2015 -->
<!-- ``` -->
...
<!-- swe_2015$kin_full %>% -->
<!--   group_by(age_focal) %>% -->
<!--   summarise(sandwiched_by_daughter = sum(living[kin=='d' & age_kin<=15]), -->
<!--             sandwiched_by_mother = sum(living[kin=='m' & age_kin>=75]), -->
<!--             sandwiched_level = sandwiched_by_daughter + sandwiched_by_mother) %>% -->
<!--   ggplot(aes(age_focal, sandwiched_level))+ -->
<!--   geom_line() + -->
<!--   scale_x_continuous(labels = seq(0,100,5), breaks = seq(0,100,5)) -->
<!-- ``` -->
