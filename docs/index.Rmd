...
- The required readings can be downloaded from: https://nextcloud.demogr.mpg.de/s/LoiF9zbE95ow7ge
...
This page will be changing during the workshop so don't be afraid if you see changes, we are making adaptations and fixing errors (hope not).
...
Packages in the `tidyverse` family will be very useful for summarize and visualizing results Load all those libraries and you should receive no error, but please contact us in case some issues come up with this topic. Packages `matrixcalc` and `Matrix` will help us with some matrix operations.

Data for examples and exercises can come from HMD and HFD collection data projects, accessible via `HMDHFDplus` package, or using the [DataPortal API](https://population.un.org/dataportal/about/dataapi) from United Nations-Population Division, or from tables located in the shared folder.

### Key vector and matrix operator in R

There is a lot of resources on matrix algebra with R ([here]() and [here]() for example), but some quick things for our purposes would be:  
...
# vectors in R are column-vectors, but dont have a dimension 5x1, are created as
# "atomic", with a legth. If we need to see the vector in two dimensions we need to
# convert him to a matrix object, in this case with 5 rows and 1 column:
# dim(a_vector) this gives error
...
```{r}
# element by element between matrices
...
- Determinant, inverse and eigen-decomposition
...
```{r}
# let´s create a (hopefully) non-singular matrix
set.seed(50)
...
### Exploring and summarising tables with `dplyr` and `tidyr`

In some exercises we will a long table with many variables and we will need to get indicators, like mean age of daughters when focal is ages 30. `dplyr` and `tidyr` packages are really good for that, and for sure you already know those, but here a quick example with `mtcars` data, getting mean age of `wt` by `cyl` and `am`, but later showing `am` cathegories in columns:
...
Finally the package `ggplot2` will be for visualize results.
