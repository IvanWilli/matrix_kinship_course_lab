...
<!-- DAG: Not sure if the following line belongs here?? Can't see the connection to daughters, since they don't need a subsidy term -->
<!-- IW:  for daughters the subsidy is the fertility of focal, Hal names subsisdy also to this fertility, no matters if the population of mothers comes from Focal or not-->
...
<!-- DAG: I don't get this question: how would you say it in Spanish? -->
<!-- IW:  si que la moda este a la derecha o a la izquierda de la edad de focal tiene sentido-->
**Answer**: Is coherent the mode of each kin distribution with Focal's age? Take a moment and do this bivariate comparison by intuition reviewing each type.
...
<!-- DAG: I think these are all good exercises but they are a lot and I am not sure that we will have time to cover everything. So, my suggestion would be to order them in such a way that the most important ones go first and the least important ones go last, known that these may not be covered given time constraints. -->
<!-- IW: yes, true. I think already are in kind of that order -->
...
With this input we can model kinship structure in APC dimensions:

[APC](apc.png)

Let's take a look at the resulting kin counts from a time-variant model for a Focal born in 1960, limiting the output to a selection of relatives (see argument `output_kin`).
...
These are the living kin that for an average woman born in 1960, given the time-variant fertility, mortality and population distribution for the 1900-2018 period. Note the argument `output_cohort = 1960`, used to extract estimates for a given cohort of Focals (a diagonal in the Lexis diagram). This is a subset from all possible results (101 age-classes and 119 years). Estimates stop at age 58 because we only provided (period) input data up to year 2018 (`2018 - 1960 = 58`).

> `DemoKin` will only return values for one or more periods OR cohorts, but never for a given period and cohort combination. This is related to time/memory issues since providing all possible period and cohort estimates can result in very large objects (for our current example, 119 X 101 x 101 x 14 ~ 17 millions rows).

For example, the following code will give an error since we are asking for **both** a cohort and period output at the same time:
...
What we can do is to compare across cohorts. We can, for example compare the 1910 and 1960 cohorts. Take some time to interpret results below (the table and plot).

**Answer**: Do you notice any unexpected trend for some kin type when comparing across cohorts? What may be causing it?
...
The function `kin` provides information on the number of relatives lost by Focal during her life, stored in the column `count_cum_death`. The plot below compares patterns of kin loss for the 1910 and 1960 cohorts.

**Answer**: Which kin types show the largest differences in terms of kin loss across the two cohorts? Discuss with regards to absolute and relative differences in the expected number of deaths by kin type.
...
Given these population-level measures, we can also compute Focal's mean age at the time of her relative's death. **Answer:** Consider a Focal aged 50 in both cohorts: how would you describe the differences in terms of her mean age at kin loss for different relatives types?
...
```{r, eval=F}
df_locations <- read.csv('https://population.un.org/dataportalapi/api/v1/locations?sort=id&format=csv', sep='|', skip=1)
View(df_locations)
```
...
# reshape fertility conveniently for DemoKin
country_fert <- data %>%
  select(age, year, fx) %>%
...
**1.1** Using the `data` object created above but for your own country, reshape the fertility and survival data in the same way in which we reshaped the fertility data above. You can plot the $q_x$ values over age for all periods available in the same way we did for Sweden (use different colours for each period).
...
**1.2** Run a time-variant kinship model for your country. Create only results for the years 1950 and 2015 (i.e., from a period perspective). Focusing on a Focal aged 50, compare both years in terms of (a) total living and and (b) mean age. **Answer:** What are two findings that you find especially interesting?
...
E.1.2$kin_summary %>%
  DemoKin::rename_kin() %>%
  ggplot(aes(age_focal,mean_age,color=factor(year))) +
  geom_line() +
  labs(y = "Expected number of living relatives") +
  facet_wrap(~kin,scales = "free")+
  theme_bw()
...
**2.1** Run a time-variant model for your country, getting only estimates for the 1990 cohort. If we assume that each kin lives all the interval and dies at the end of the year (if at all) , this means that each lived year by each kin was a year "shared" between Focal and than relative. If this is true, you can sum kin availability to get an approximation of the cummulative number of person-years of share lifetime between Focal and her mothers, children, etc.

> Take a look to the graphic from @Song2019 for shared grandchild-years from a grandparent perspective, for this exercise, we are using the definition number 2.

![Fig. 1 in Song (2019)](song_shared.png)

**Answer:** How many person-years were shared between Focal and her sisters between Focal's birth and Focal's 15th birthday?

<!-- DAG: There is something weird going on here: the output of kin shows no relatives for most ages of Focal?? -->
<!-- IW: Should show only sisters. I think works -->
```{r, eval= F, include=F}
E.1.3 <-
  kin(p = country_surv,
    f = country_fert,
...
E.1.3$kin_summary %>%
  ...
  filter(age_focal == 15) %>%
  summarise(sister_years =sum(shared_years), .by = c(cohort, kin))
```

#### 3. Covid bereavement

The Covid pandemic had a big impact in kin bereavement for some countries (@verdery_tracking_2020; @Snyder2022). We will use `DemoKin` to approximate the scale of this 'excess bereavement'. For this, you will proceed in steps:

1. You will create a 'baseline' of what bereavement would have looked like in 2020 and 2021 in the absence of COVID. A quick and dirty way of doing this is to create counterfactuals by assuming that the mortality levels observed in 2019 (i.e. before the pandemic) were in effect during 2020-2021. You can also create more sophisticated baselines if you want.
1. As a first step visualise the excess mortality implied in the survival pattern for the 2020-2021 years. This will helpful for anticipating the the magnitude of the bereavement shock you can expect.
1. Run two time-variant kinship models: one 'realistic' model that uses the empirical mortality and fertility rates for the 1950:2021 period. The 'counterfactual' models is equivalent to the 'realistic' one except that it uses the 2019 mortality rates for the 2020:2021 period.

**3.1**. Using these two models, estimate difference between living older kin for a female aged 60 in the 'realistic' and 'counterfactual' scenarios.

<!-- DAG: The results of this exercise are also 0??-->
<!-- IW: Strange, is not 0 for me-->
```{r, eval= F, include=F}
...
c(E.1.4$kin_full %>%
  summarise(all_kin = sum(living[age_focal == 60 & age_kin>60])),
...
![Caswell, 2021. Fig 2.](time_var_boundaries.png){width=80%, height=80%}
...
D_table %>% rename(living_by_hand = count_living) %>% 
  inner_join(
...
<!-- During this section, we will use `DemoKin` for **Age-classified time-invariant two-sex** kinship models and do some exercises. -->
...
<!-- <!-- A question about how to relate with previous models: should the total living aunts be the same number than the one from the female one-sex model? And daughters? --> -->
<!-- <!-- IW: no, because now you have aunts from father´s side too. Daughters yes because all daughters comes from focal´s fertility--> -->
...
<!-- For this, we extend the period using data located in *"data/fra_2sex.Rdata"*, that you can load with function `load` as we did in previous days. -->
<!-- For this example, we will 'pretend' that male fertility rates are the same than fertility but slightly older, translating shape for the difference in the mean age observed in 2012 (taht you calcculated before). -->
...
<!-- **E1.1** In a time invariant model: how many living grand-mothers and grand-fathers does a female expects to have at age 15 in 1950 and in 2015, and what are their mean ages? Extract a conclusion based on the results. -->
...
<!--   ) -->
<!-- ``` -->
<!-- **E1.2** Compare results using observed population for each sex. Why is this different? -->


<!-- **E1.2** Compare sex ratios of grandparents, parents and daughters in a time invariant and in a time-variant framework for the cohort 1950. -->

<!-- #### Modelled and observed kinship structures -->
...
<!-- <!-- IW: I think we can add a variant with time-variant one sex, or replace by the 1 --> -->
<!-- ```{r} -->
<!-- data.frame( -->
<!-- `Model ID` = 1:3 -->
...
<!-- Data for female mortality and fertility are already included in `DemoKin`. For males you can access to a file called "swe_males.Rdata" in the data folder, with age-specific survival probabilities and fertility rates. Where comes from this data? -->

<!-- - Male fertility can be downloaded from [Human Fertility Collection](https://www.fertilitydata.org/Home/Index) site, directly from R with sentence `read.table("https://www.fertilitydata.org/File/GetFile/Country/males/SWE/m_SWE_ASFRstand_TOT.txt", sep = ",", header = T)`. -->

<!-- - Male survival was downladed from HMD using package `HMDHFDplus`, take a look [here](https://cran.r-project.org/web/packages/HMDHFDplus/HMDHFDplus.pdf). -->

<!-- When arranging inputs for using in DemoKin, be aware that female and male data have different window range, so take the interval where you have data for both. -->

<!-- Using the model outputs, answer: -->

<!-- **E2.1** How well do your models approximate the kin counts reported by Kolk et al. (2021)? Justify your answer using quantitative measures (max 250 words and 4 figures) -->
...
<!-- # we´ll compare until 2015 againts 2017 hoping no big diferences because of that. Other option is to mantain 2015 rates two more years. -->
...
<!--            kin = unique(kin_out_time_variant$kin_summary$kin)) -->

<!-- # arrange my data to compare -->
<!-- df_comparison <- bind_rows( -->
<!--   Model_1 %>% mutate(model = "gkp"), -->
<!--   Model_2 %>% mutate(model = "androgynous"), -->
<!--   Model_1 %>% mutate(model = "full")) %>% -->
<!--   inner_join(kin_types_kolk) %>% -->
<!--   mutate(cohort = 2017-age_focal) %>% -->
<!--   summarise(living = sum(count_living), .by = c(kolk_kin, cohort, model)) %>% -->
<!--   bind_rows(kolk %>% rename(living = mean, kolk_kin = kin) %>% mutate(model = "Kolk")) -->

<!-- # plot -->
<!-- df_comparison %>% -->
<!--   filter(!is.na(kolk_kin)) %>% -->
<!--   ggplot(aes(x = cohort, y = living, color = model)) + -->
<!--   geom_line()+ -->
<!--   theme_bw() + -->
<!--   facet_wrap(~kolk_kin) -->
<!-- ``` -->

<!-- **E2.2** Based on this exercise, discuss the advantages and drawbacks of using formal models to quantify kinship (max 250 words) -->
...
<!-- toy_ex$kin_summary %>% filter(kin == "m") -->
<!-- db_mothers; db_fathers -->
<!-- toy_ex$kin_summary %>% filter(kin == "d") %>% -->
<!--   summarise(living = sum(count_living), .by = c(age_focal, sex_kin)) -->
<!-- a_daughter; a_son -->
<!-- ``` -->
