...
- Syllabus: https://github.com/IvanWilli/matrix_kinship_course_lab/syllabus.
- The required readings and course's slides can be downloaded from: https://www.dropbox.com/t/iM06GB0D5fXboWYu.
- Find this website's source code on GitHub: https://github.com/IvanWilli/matrix_kinship_course_lab.
- Data for exercises can be found in previous dropbox folder or in https://github.com/IvanWilli/matrix_kinship_course_lab/data.

This page will be changing during the workshop so don't be afraid if you see some movements, we are making adapting content or fixing errors (hope not).
...
Packages in the `tidyverse` family will be very useful for summarize and visualizing results. Please load those libraries and you should receive no error, but please contact us in case some issues came up with this topic. Packages `matrixcalc` and `Matrix` will help us with some matrix operations.

Data for examples and exercises may come from UN population estimates and projections, using the [DataPortal API](https://population.un.org/dataportal/about/dataapi) from United Nations-Population Division, or from tables located in the shared folder (see \@ref{tab:Introduction}).

### Operations with vector and matrix in R

The intention of this section is to introduce some basic operators of matrices and vectors that are introduced and deepened in the theoretical part. There is a lot of online resources on matrix algebra with R ([here](https://www.taylorfrancis.com/books/mono/10.1201/9781315370200/basics-matrix-algebra-statistics-nick-fieller) a most comprehensive one), here we are going to show some relevant ones for our purposes. See [@Caswell2001] for mathematical details.  
...
# vectors in R are column vectors, but don´t have a dimension by default, these are created as
# "atomic", with a length. If we need to see the vector in two dimensions we need to
# convert it to a matrix object with as.matrix(a_vector). 
# Short story: consider a vector always a column vector but check its dimension with "length" function.
length(a_vector)
```

> Checking dimenions is a helpful information for validate if two objects can be multiplied and in which order. You know something related to that goes wrong when you see this message "non-conformable arguments".
...
We´ll use this in *By hand* sections, and is used a lot inside `DemoKin`. Here a very intuitive figure multiplying $A$ by $B$: 
...
Don´t confuse with the *Hadamard* product, the element by element one which use the operator `*`.

```{r}
a_matrix * other_matrix # column-sum
a_vector * other_vector # row-sum
```

- Determinant, inverse and eigendecomposition

Let´s create a *non-singular* matrix (with no zero determinant, invertible):
```{r}
set.seed(50)
...
Linear algebra is useful for finding a vector $x$ that pre-multiplied by $A$ produce a vector $b$, in the form of a linear system of equations $Ax=b$, and finding the solution with $x=A^{-1}b$ (see [@Caswell2001] for details about when this have an unique, none or infinite solutions). As an example let´s solve this situation: *"I have 6 nephews and 10 nieces. My brothers had in average 1 son and 2 daughters and my sisters 1.5 sons and 2 daughters. How many brothers and sister do I have?"*. 
...
A = matrix(c(1, 1.5, # My brother had in average 1 son and my sister 1.5
           ...
           2, 2, byrow = T)
x = solve(A) %*% b # amount of brothers and sisters
```
...
Check more on transforming with [dplyr](https://dplyr.tidyverse.org/articles/programming.html) and reshaping with [tidyr](https://tidyr.tidyverse.org/articles/pivot.html) if you are curious. Finally the package [ggplot2](https://ggplot2.tidyverse.org/) will be for visualize results. A veeery basic one:
...
> Of course, in the exercises you can use the tools that you want!
...
During this section the idea is to have the sense of how translate theory of **Age-classified time-invariant one-sex kinship models** to code, how to use `DemoKin` package, and finally do some exercises.
...
The argument **birth_female** is set as 1/2.04 by default, and this multiplies `f` argument at some point, so in case you are using fertility rates already for female offspring only this parameter needs to be set to 1. 
...
A lot of interesting data to explore. Let´s give more context and set some research questions as an example.
...
Let´s use the pre-loaded Swedish data.
...
We can visualize the implied kin counts for a Focal woman aged 35 yo in a time-invariant population using a network or 'Keyfitz' kinship diagram [@Keyfitz2005] with the `plot_diagram` function. Let´s take a moment to see what means each value.
