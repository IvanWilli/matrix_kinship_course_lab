...
Let´s keep studying our female closed population with 5 age-classes, but now we know that in next 9 years ($T=9$) is expected that probability of death will decline 1% by year for all age-classes, and fertility rates will decline 1% by year too. We use $Pt$ and $Ft$ for create matrices with time varying rates (time in columns and age in rows).
...
In this case we had access to a vector female population `pop` at the beginning of the period, so we can project this and calculate empirical mother´s distribution by age for that year and next ones:
...
These are the living kin that expects to have an average woman born in 1960, given experienced fertility, mortality and population distribution since 1900 to 2018. Here is important to distinguish between time boundary restriction and cohort year: with `output_cohort = 1960` we are sub-setting a Focal cohort (a diagonal) from all possible results (101 age-classes and 119 years). Estimates stop at age 58 because of input restriction up to 2018.
