...
Remember each *age* is a *stage*, so age 0 is the stage 1, age 1 is the stage 2, and so on. At stage 5 (age 4) nobody is alive ($\omega$ in life table applications).
...
Let´s build matrices $U$ and $F$. For R, the letter $F$ is reserved for `FALSE`, so can not use it as it is, in our case we add a point. A note: 
...
Now find eigenvalues and eigenvectors to know $w$ the stable distribution by age, and then $\pi$ the one for the mothers.
...
Now we are going to see how project some relatives. For each of those we are going to create a matrix of dimension $5$x$5$ (or $\omega$x$\omega$ in general) to save the vectors of surviving kin by age (rows) in each Focal´s age (columns). 
...
In this case, $\pi$ is the age distribution of mothers when Focal just born. Remember, every mother is alive a that moment, so the total (sum of $\pi$) should be 1. This distribution will be placed in the first column of $D$.
...
E <- diag(1, ages)
for(x in 2:ages){
  A[,x] <- U %*% A[,x-1] + F. %*% E[,x-1] # eq 6 in Caswell (2019) but in for each column in this matrix
}
...
The treatment is different depending if the sister born before or after Focal.
  ...
  - Older sisters: the initial distribution by age comes from the probability that Focal´s mother having a daughter at the time that Focal born, and then just survive (no subsidy, no new old sisters).
...
First please load the libraries:
...
Some questions arise: Is there a pick in the total? How about composition, which colors prevails in each life stage?
...
Is coherent the mode of each kin distribution with Focal´s age? Make this bivariate comparison by intuition reviewing each type.
...
During this section the idea is to have a sense of how translate theory of Age-classified time-**variant** one-sex kinship models to code, how to use `DemoKin` package for that goal, and do some exercises.

### By hand

Let´s keep studying our female closed population with 5 ages (from 0 to 4), but now we know that in next 9 years is expected that probability of death will decline 1% by year for all ages, and fertility rates will decline 1% by year too. We use $Pt$ and $Ft$ for create matrices with time varying rates (time in columns). $T$ is the time interval (in years, in principle) for which we have data.
...
Those matrices are not convenient for projection purpose, because we need to create transition matrices. Let´s create two lists with time dependent matrices for survival and fertility, called $Ut_list$ and $Ft_list$, that will be used soon.
```{r}
...
In this case we had access to vector female population, at the beginning of the period so we can project this and calculate empirical mother´s distribution by age for that year and next ones:
...
One solution to the time boundary restriction comes with assuming a stable population behaviour before $t=0$: every Focal has living kin with the pattern by age that we saw in time-invariant method. What would be another solution? 

The solution to the age boundary will come from taking the empirical $\pi(t)$ at each time (also stable assumption can be done in case no observed data is available, but is not the same). And taht´s the reason why we saved $\pi$ for all the years.

Let´s start in base year ($t=0$), define a list that will save mothers by age (rows) by Focal´s age (cols) in each year, as we did in the previous day, and assign the distribution that comes from stable assumption:
...
for(x in 2:ages){
  D0[,x] <- Ut_list[[1]] %*% D0[,x-1]
}
```

So this is the inital snapshot of mothers by age of Focal in base year. Every average woman by age in that year experimented what Focal did in the time-invariant scenario.
...
Let´s create a list for saving each $Dt$ matrix of living mothers at each year:
...
> Consider that we are creating a series of Focal's cohorts: at each year $t$ we have 4 Focals, one at each age (not 5 because $\omega=5$) with living kin distribution by age and type that depends of experienced rates.

Let´s bind results in a table (data frame), so we can see what we have.

```{r}
D_table <-lapply(1:length(Dt_list), function(t){
   data.frame(age_focal = age, year = t-1, Dt_list[[t]])}) %>%
  dplyr::bind_rows() %>%
  tidyr::pivot_longer(X1:X5, names_to = "age_kin", values_to = "living") %>%
  dplyr::mutate(age_kin = as.integer(substr(age_kin,2,2))-1)
```

We have a table with 250 rows (5 ages of Focal by 10 years by 5 ages of mother), with living mothers by focal´s age, in each year, depending kin´s age, which allows analysis in APC dimensions (section 2.2 in @caswell_formal_2021). This is pretty similar to what `Demokin` give us as output in `kin_full` data frame.
...
These are the living kin that expects to have an average woman born in 1960, given experienced fertility, mortality and population distribution since 1900 to 2018. Here is important to distinguish between time boundary restriction and cohort year: with `output_cohort = 1960` we are sub-setting a Focal cohort (a diagonal) from all possible results (100 ages and 119 years). Estimates stop at age 58 because of input restriction up to 2018. 

> Actually `DemoKin` can not return all APC combinations because of time/memory performance issues related to the size of data (119 X 101 x 101 x 14 ~ 17 millions of rows). One option is only return `kin_summary` in this cases. This limitation could a programming skills limitation too ;) (sorry for that).
...
**E.1** Thanks to [DataPortal API](https://population.un.org/dataportal/about/dataapi#tutorial-using-r) from UN we can download estimated rates from any country in the world, produced by the [World Population Prospects](https://population.un.org/wpp/) project. Pick your country and download the data with the function `get_UNWPP_inputs` (located with this Rmd [here](https://github.com/IvanWilli/matrix_kinship_course_lab/tree/main/docs)).
...
**E.1.1** Reshape data for fertility.
...
**E.1.2** Run a time-variant model for your country, selecting years 1950 and 2015 from a period perspective.

```{r, eval = F}
E.1.2.country_time_varying_period <-
  kin(
    p = country_surv,
    f = country_fert,
    time_invariant =FALSE,
...
E.1.2.country_time_varying_period$kin_summary %>%
  # DemoKin::rename_kin() %>%
...
E.1.3 What questions related to covid-19 impact do you think are interesting to test? If so, first plot rates to see if actually an impact can be detected.

E.2 for which situation affects closed population assumption. migration of realtives can modify average pattern initially from sweden, sistematic migration for daughters students

E.3 Run toy example with DemoKin and check if you have same results.
