...
It has year in columns and age in rows. And plotted $q_x$ ($´s complement) over age in a year, for example 2015, looks like this:
...
Let´s keep studying our female closed population with 5 ages (from 0 to 4), but now we know that in next 10 years is expected that probability of death will decline 1% by year for all ages, and fertility rates will decline 1% by year too. We use $Pt$ and $Ft$ because are both a matrix with time varying rates (time in columns).

```{r}
age <- 0:4
ages <- length(age)
T. <- 10 # again T is reserved for logical TRUE value, so we add a point
years <- length(0:T.)
p <- c(.9, .7, .4, .1, 0)
...
Now let´s create two lists with time dependent $U$ and $F$ ($Ut_list$ and $Ft_list$), that will be used soon.
```{r}
...
for(t in 1:years){
  # create matrices
  U_temp[row(U_temp)-1 == col(U_temp)] <- Pt[,t]
  F_temp[1,] <- Ft[,t]
  # assign
  Ut_list[[t]]<- U_temp
  Ft_list[[t]] <- F_temp
}
# see survival matrix at t=2
Ut_list[[2]]
```

In this case we had access to female population at the beginning of the period $z$ so we can project this, and calculate empirical mother´s distribution by age for that year and project for the others:
...
# distribution by age
z <- pop / sum(pop)
# project population Z
Z <- Pi <- matrix(0, ages, years+1)
Z[,1] <- z
...
> Based on classes from population matrix: what will be the growth rate of this population in the stable stage?

Basically this is what we are going to do for kinship *populations*: make dynamic matrices by time with those lists. 

For each kin we need to define the initial snapshot by age at time 0 $k(x,0)$, and what is the initial kin availability for years 0 to $T$ (eq. 15 and 16 from @caswell_formal_2021).

![Caswell, 2021. Fig 2.](time_var_boundaries.png)

- **Mother**

Left boundary (previous to observed data) comes from stable population assumption: every Focal has a mother with the pattern by age that we saw in time-invariant method. Bottom boundary, boundary by time will come from empirical $\pi$ at each time (also stable assumption can be done in case no observed data is available, but is not the same). Remember that as a first step we need the mother distribution by age of Focal at every age.

Let´s start in base year (t=0, left boundary), defining a list that will save mothers by age (rows) by Focal´s age (cols) in each year:
...
So this is the inital snapshot in base year (left boundary) of mothers by age of Focal. Every average woman by age in the base year experimented what Focal did in the time-invariant scenario. 
...
Let´s create a list for saving each $Mt$ matrix of living mothers at each year:
...
We have a table with 275 rows (5 x 11 x 5), with living mothers by focal´s age, in each year, depending kin´s age, which allows analysis in APC dimensions (section 2.2 in @caswell_formal_2021). This is pretty similar to what `Demokin` give us as output in `fin_full` data frame. 

Similar reasoning follows other kin types. Left boundary comes from time-invariant assumption and bottom boundary comes $\pi(t)$ at each year and relates associated with those ages in same time.
...
swe_pop %>% as.data.frame() %>% 
     mutate(ages = 1:nrow(swe_asfr)-1) %>% 
     pivot_longer(-ages, names_to = "year", values_to = "pop") %>%
     mutate(year = as.integer(year)) %>% 
     ggplot() + geom_tile(aes(x = year, y = ages, fill = pop)) + 
     scale_x_continuous(breaks = seq(1900,2020,10), labels = seq(1900,2020,10))
```

We account for this, by using the time-variant models introduced by @caswell_formal_2021. Let's take a look at the resulting kin counts for a Focal born in 1960, limiting the output to the relative types given in the argument `output_kin`. Take some time...
...
These are female living kin what expects to have an average woman born in 1960, given current fertility, mortality and population distribution since 1900 to 2018. here is important to distinguish between left boundary restriction and cohort year: `output_cohort = 1960` we are subletting all possible results. Estimates stop at age 58, because of input restriction up to 2018. Actually `DemoKin` can not give in return all combinations because of time performance issues related to the amount of data to that culd be returned. Maybe is a limitation from programmer´s (sorry for that). 

But we can compare cohorts, for example against 1910`s one. Take a time to interpret results (table and plot).
...
But maybe you are interested in take a snapshot of kin distribution in some year, for example 1990. We use `output_period = 1990` for that:
...
The function `kin` also includes information on the number of relatives lost by Focal during her life, stored in the column `count_cum_death`. Let´s follow with the cohort comparison.
...
E.1 calculate pi for by hand and compare with empirical distribution
